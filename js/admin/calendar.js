...
var CalendarSessionHandler = {
	newSession: function(values, successCallback, errorCallback, options){
		...
		popup({
            ...
            content: function(formCreator){
				...
				form += formCreator.rowForm("Học sinh:", elementCreator.input({"id":"ajaxSearchStudent", "type":"text", "class":"form-control class_email"}));
				...
				form += formCreator.rowForm("Ngày học:", elementCreator.input({"type":"text", "name":"Session[plan_start]", "value":day, "readonly":true}));
				form += formCreator.rowForm("Thời gian:", 
	...
	editSession : function(data, successCallback, errorCallback, options){
		...
		popup({
            ...
            content: function(formCreator){
                ...
                form += formCreator.rowForm("Chủ đề buổi học:",elementCreator.input({"type":"text","name":"Session[subject]", value:data.subject}));
				form += formCreator.rowForm("Giáo viên:", data.teacherName);
				form += formCreator.rowForm("Học sinh:", elementCreator.input({"id":"ajaxSearchStudent","type":"text", "class":"class_email", value:data.student}));
				form += formCreator.rowForm("Khóa học:", elementCreator.select({"id":"courseSelect", "name":"Session[course_id]"}, elementCreator.option("", data.course_id)));
				form += formCreator.rowForm("Ngày học:", day);
				form += formCreator.rowForm("Thời gian:", startHour + '&nbspGiờ&nbsp&nbsp' + startMin + ' Phút');
				...
				switch (data.className[0]){
					case 'ongoingSession':
						form += formCreator.rowForm("","<a href='#' onclick='endSession("+ data.sessionId +", "+ data.end +");removePopupByID"+'("popupAll")'+";return false;'>Kết thúc buổi học</a>");
						break;
					case 'approvedSession':
						form += formCreator.rowForm("","<a href='/admin/session/cancel/id/"+ data.sessionId +"' target='_blank'>Hủy buổi học</a>");
						break;
					case 'pendingSession':
						form += formCreator.rowForm("","<a href='#' onclick='approveSession("+ data.sessionId +");removePopupByID"+'("popupAll")'+";return false;'>Xác nhận buổi học</a>");
						break;
					default:
						break;
				}
				if (data.className[0] != 'ongoingSession'){
					form += formCreator.rowForm("","<a href='#' onclick='deleteSession("+ data.sessionId +");removePopupByID"+'("popupAll")'+";return false;' style='color:red'>Xóa buổi học</a>");
				}
				form += formCreator.rowForm("&nbsp;","<button id='saveSession'>Lưu lại</button>" +
