...
$.extend(Grid.prototype, {
	...
	getEventSkinCss: function(event) {
		...
		if (event.className.indexOf('reservedSlot') > -1 && event.start.isBefore(today)){
			var closedBgColor = 'darkgrey';
			event.title = 'Closed';
			event.className = 'reservedSlot closed';
		}
				...
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
						'<span>' + // for matchCellWidths
							htmlEscape(calendar.formatDate(slotDate, view.opt('axisFormat'))) + ' - ' + htmlEscape(calendar.formatDate(slotEnd, view.opt('axisFormat'))) +
						'</span>' :
						''
						) +
				'</td>';
		...
		for (i = 0; i < segs.length; i++) {
				...
				for (x in slots){
					...
					if (timeConflict){
						...
						if (reservedSlot){
							addFlag = false;
							break;
						} else if (!reservedSlot && slots[x].placeholder) {
							// console.log('kill it');
			...
			if (addFlag) {
				...
				var slot = {
					...
					segment: segs[i],
					placeholder:(segs[i].event.className.indexOf('reservedSlot') > -1) ? true : false,
				}
