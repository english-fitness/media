<?php
session_start();
$userId = isset($_SESSION['loggedUserId'])? $_SESSION['loggedUserId']: null;
$checkAccessUploadUser = isset($_SESSION['checkAccessUploadUsers'])? $_SESSION['checkAccessUploadUsers']: false;
if(isset($_GET['user_id']) && $_GET['user_id']!="" && $checkAccessUploadUser){
	$userId = $_GET['user_id'];
	$_SESSION['current_dir_user'] = $userId;//Current dir user
}
if($userId==null){
	header('location:/');
}
...
$base_url ="http".$https."://".$_SERVER['HTTP_HOST'];  // DON'T TOUCH (base url (only domain) of site (without final /)).
$upload_dir = '/media/uploads/users/'.$userId.'/'; // path from base_url to base of upload folder (with start and final /)
$current_path = '../../uploads/users/'.$userId.'/'; // relative path from filemanager folder to upload folder (with final /)
