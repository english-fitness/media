...
if (isset($_GET['action']))
{
    switch($_GET['action'])
    {
        ...
        case 'save_text_file':
            ...
            break;
		case 'getContent':
			if (file_exists($current_path . $_POST['path']) && createLink($_POST['path'], $own_dir, $library_folder)){
				response('File have been copied successfully')->send();
				exit;
			} else {
				response('Unexpected error happened. Please try again later')->send();
				exit;
			}
			break;
        default:
