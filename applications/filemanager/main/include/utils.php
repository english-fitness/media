...
function createLink($target, $folder, $target_folder_link){
	//extract filename
	$lastSlashPos = strrpos($target, '/');
	if ($lastSlashPos !== false){
		$filename = substr($target, $lastSlashPos + 1);
	} else {
		$filename = $target;
	}
	...
	if (file_exists($folder . $symlinkName)){
		$i = 1;
		while(file_exists($folder . '_' . $i . $symlinkName)){
			$i++;
		}
		$symlinkName = $i . '_' . $symlinkName;
	}
	
	$symlinkTarget = $target_folder_link . $target;
	
	return symlink($symlinkTarget, $folder . $symlinkName);
}
